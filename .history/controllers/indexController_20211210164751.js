         ...
         if(NewPass==confirm_password){
             ...
             TrainerModel.default.findById(req.user._id, (err, user)=>{
                 if(err) console.log(err);
                   ...
                   {
                       // Bcrypt middleware
TrainerModel.Trainer.default.pre('save', function(next) {
    var user = this;
    ...
    bcrypt.genSalt(SALT_WORK_FACTOR, function(err, salt) {
        ...
        TrainerModel.methods.comparePassword = function (OldPass, cb) {
            bcrypt.compare(OldPass, this.password, function (err, isMatch) {
                if (err) return res.send('-3');
                cb(null, isMatch);
            });
        };
                       // Password verification
                       /*
                        if(OldPass.trim()!==req.user.password.trim()){
                            // -2: server error
                            return res.send('-2')
                        }
                        */
